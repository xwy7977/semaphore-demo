...
int main() {
    ...
    printf("Process B %d successfully opened semaphores\n", getpid());

    // Keep the process alive using an infinite loop
    while (1) {
        sem_wait(semA); // P operation
        i++;
        printf("Process B %d entered critical section for the %d time\n", getpid(), i);
        sleep(10); // Print every 10 seconds to avoid high CPU usage
        printf("Process B %d leaving critical section\n", getpid());
        sem_post(semB); // V operation
        if (i >= 5)
        {
            printf("Process B %d finished 5 operations, preparing to exit\n", getpid());
            break; // Exit loop
        }
